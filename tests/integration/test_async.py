...
Every child here is closed rather than dropped: the first await binds an asyncio
read transport to the spawn and keeps it for the next one, and closing the spawn
is what releases it. See the same note in tests/test_async.py.
"""
...
class AsyncTests(pexpect_test_case.AsyncPexpectTestCase):
    ...
    async def test_async_and_gc(self) -> None:
        ...
        with pexpect.spawn(f"{self.PYTHONBIN} sleep_for.py 0.05", encoding="utf8") as p:
...
class AsyncREPLWrapTests(pexpect_test_case.AsyncPexpectTestCase):
    ...
    async def test_async_replwrap(self) -> None:
        ...
        bash = replwrap.bash()
        self.addCleanup(bash.child.close)
...
class AsyncInterruptTests(pexpect_test_case.AsyncPexpectTestCase):
    ...
    async def test_recovers_when_no_prompt_follows_the_signal(self) -> None:
        ...
        """
        repl = no_editor_repl()
        self.addCleanup(repl.child.close)
