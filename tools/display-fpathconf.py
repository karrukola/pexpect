...
DISP_VALUES = (
    ("PC_MAX_CANON", "Max no. of bytes in a terminal canonical input line."),
    ("PC_MAX_INPUT", "Max no. of bytes for which space is available in a terminal input queue."),
    ("PC_PIPE_BUF", "Max no. of bytes which will be written atomically to a pipe."),
    ("PC_VDISABLE", "Terminal character disabling value."),
)
FMT = "{name:<13} {value:<5} {description}"


def display_fpathconf() -> None:
    """Print each terminal-related fpathconf value for stdin."""
    # column header
