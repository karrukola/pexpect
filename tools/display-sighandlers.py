#!/usr/bin/env python
"""Display all signals, their values, and their handlers."""
...
for name, value in [
    ...
    if signal_name.startswith("SIG") and not signal_name.startswith("SIG_")
]:
    try:
    ...
    description = {signal.SIG_IGN: "ignored(SIG_IGN)", signal.SIG_DFL: "default(SIG_DFL)"}.get(
        handler, handler
    )
    print(FMT.format(name=name, value=value, description=description))
