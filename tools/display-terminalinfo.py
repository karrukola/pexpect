...
BITMASK_FMT = "{name:>{col1_width}} {value:>{col2_width}}   {description}"
CTLCHAR_FMT = "{idx:<{col1_width}}   {name:<{col2_width}} {value}"
CONF_FMT = "{name:>{col1_width}}   {value}"


def display_bitmask(kind, bitmap, value) -> None:
    ...
    print(
        "{} {}   {}".format(
            "-" * col1_width, "-" * col2_width, "-" * max(map(len, bitmap.values()))
        )
    )
...
def display_ctl_chars(index, cc) -> None:
    ...
    print(
        CTLCHAR_FMT.format(
            idx="Index", name="Name", value="Value", col1_width=col1_width, col2_width=col2_width
        )
    )
    print("{}   {} {}".format("-" * col1_width, "-" * col2_width, "-" * 10))
...
def display_conf(kind, names, getter) -> None:
    """Display each name in ``names`` alongside the value ``getter`` returns for it."""
    col1_width = max(map(len, names))
    print(CONF_FMT.format(name=kind, value="value", col1_width=col1_width))
    ...
    for name in names:
        try:
            ...
            value = err
        print(CONF_FMT.format(name=name, value=value, col1_width=col1_width))
    print()


def main() -> None:
    """Dump stdin's pathconf limits, termios modes and control characters."""
    fd = sys.stdin.fileno()
    ...
    display_conf(
        kind="pathconf", names=os.pathconf_names, getter=lambda name: os.fpathconf(fd, name)
    )
    ...
    try:
        ...
        try:
            ...
            print(f"Error inspecting TTY: {e}")
